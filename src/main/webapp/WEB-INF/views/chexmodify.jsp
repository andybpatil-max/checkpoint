...
--%>
	...
	<c:if test='${chexSelector.accessFlag!="inq"}'>
	    <h1>
		<stripes:label for="check"/> <stripes:label for="data"/> <stripes:label for="maint"/>
	    </h1>
	</c:if>
	...
	<div id="detail">
		...
		<c:if test='${chexSelector.accessFlag!="inq"}'>
			...
			<th class="header" colspan="2">
			    <font size=3 color=blue>
			    	<stripes:label for="modify"/> <stripes:label for="check"/> 
				<stripes:label for="data"/>
			    </font>
			</th>
		    </tr>
		</c:if>
		...
		<tr>
		    <th align='right' bgcolor='lightyellow' width='25%' height='19'><b>
			<stripes:label for="account"/></b></th>
		    <td align='left' width='20%' height='19'>
		    	<c:if test='${chexSelector.actionFlag=="Modify"}'>
			    <stripes:text name="chexDetail.chex_account_num"
				value="${chexDetail.chex_account_num}" size='22' />
			</c:if>
			<c:if test='${chexSelector.actionFlag!="Modify"}'>
			    <stripes:text name="chexDetail.chex_account_num"
		...
		<tr>
			...
			<c:if test='${chexSelector.actionFlag=="Modify"}'>
			    <stripes:text name="chexDetail.chex_check_num"
				  value="${chexDetail.chex_check_num}" size='20' maxlength='20'/>
			</c:if>
			<c:if test='${chexSelector.actionFlag!="Modify"}'>
			    <stripes:text name="chexDetail.chex_check_num"
				value="${chexDetail.chex_check_num}" size='20' readonly='true' />
			</c:if>
		    </td>
		</tr>
		<tr>
		...
		<tr>
			...
			<c:if test='${chexSelector.actionFlag=="Modify"}'>
			    <stripes:text name="chexDetail.chex_check_amount"
				  value="${chexDetail.chex_check_amount}" size='15' />
			</c:if>
			<c:if test='${chexSelector.actionFlag!="Modify"}'>
			    <stripes:text name="chexDetail.chex_check_amount"
		...
		<tr>
			...
			<c:if test='${chexSelector.actionFlag=="Modify"}'>
				...
				value="${chexDetail.chex_issue_date}" size='10' maxlength='8'/></td>
			</c:if>
			<c:if test='${chexSelector.actionFlag!="Modify"}'>
			    <stripes:text name="chexDetail.chex_issue_date"
				value="${chexDetail.chex_issue_date}" size='10' readonly='true'/></td>
			</c:if>
		</tr>
		<tr>
		    <th align='right' bgcolor='lightyellow' width='25%' height='19'>
		...
		<tr>
			...
			<c:if test='${chexSelector.actionFlag=="Modify"}'>
				...
				value="${chexDetail.chex_payee_addr1}" size='45' maxlength='35'/>
			</c:if>
			<c:if test='${chexSelector.actionFlag!="Modify"}'>
			    <stripes:text name="chexDetail.chex_payee_addr1"
		...
		<tr>
			...
			<c:if test='${chexSelector.actionFlag=="Modify"}'>
				...
				value="${chexDetail.chex_payee_addr3}" size='45' maxlength='35'/>
			</c:if>
			<c:if test='${chexSelector.actionFlag!="Modify"}'>
			    <stripes:text name="chexDetail.chex_payee_addr3"
...
--%>
		...
		<tr>
		    <th align='right' bgcolor='lightyellow' width='35%' height='19'>
			<stripes:label for="return"/> <stripes:label for="status"/></th>
		    <td align='left' height='15' width='20%'>
			<c:if test='${chexSelector.actionFlag=="Modify"}'>
			    <stripes:text name="chexDetail.chex_return_status" value="${chexDetail.chex_return_status}" 
					size='80' maxlength='2'/>
			</c:if>
			 <c:if test='${chexSelector.actionFlag!="Modify"}'>
			    <stripes:text name="chexDetail.chex_return_status" value="${chexDetail.chex_return_status}"
		...
		<tr>
		    ...
		    <td align='left' height='15' wirth='20%'>
		    	<c:if test="${chexDetail.chex_reason_codes==' '}">
			    <stripes:text name="chexDetail.chex_reason_codes"
				value="   " size='3' readonly='true'/>
			</c:if>
		    	<c:if test="${chexDetail.chex_reason_codes!=' '}">
			    <stripes:text name="chexDetail.chex_reason_codes"
...
--%>
	    <a href="javascript:big('<c:out value="${chexDetail.chex_image_f}"/>')" onmouseout="big_hide()">
			<img border="0" src='<c:out value="${chexDetail.chex_image_f}"/>' width="40%"></a>
	    <a href="javascript:big('<c:out value="${chexDetail.chex_image_b}"/>')" onmouseout="big_hide()">
			<img border="0" src='<c:out value="${chexDetail.chex_image_b}"/>' width="40%"></a>
	</center>
	<br>
<%--													--%>
...
--%>
	<center>
	    ...
	    <c:if test='${chexSelector.actionFlag=="Update"}'>
		<stripes:submit name="Confirm" value="Confirm Update Check"/>
	    </c:if>
<%--
	    <c:if test='${chexSelector.accessFlag=="inq"}'>
		<c:if test='${chexSelector.detail_count gt 0}'>
