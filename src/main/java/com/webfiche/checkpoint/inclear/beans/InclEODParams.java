...
public final class InclEODParams implements Serializable {
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private String className		= "> InclEODParams.";
	...
	public void ShowDetails () {
		...
		PrintLog("");
		PrintLog("datadir			= " + datadir);
		PrintLog("reportdir			= " + reportdir);
		PrintLog("imagedir			= " + imagedir);
		PrintLog("product_id		= " + product_id);
		PrintLog("our_aba			= " + our_aba);
		PrintLog("bankId			= " + bankId);
		PrintLog("eod_status		= " + eod_status);
		PrintLog("eod_flag			= " + eod_flag);
		PrintLog("bod_flag			= " + bod_flag);
		PrintLog("file_loaded		= " + file_loaded);
		PrintLog("fileLoaded1		= " + fileLoaded1);
		PrintLog("fileLoaded2		= " + fileLoaded2);
		PrintLog("fileLoaded3		= " + fileLoaded3);
		PrintLog("fileLoaded4		= " + fileLoaded4);
		PrintLog("fileLoaded5		= " + fileLoaded5);
		PrintLog("currency			= " + currency);
		PrintLog("createNewHistory	= " + createNewHistory);
		PrintLog("today				= " + today);
		PrintLog("prev_biz_date		= " + prev_biz_date);
		PrintLog("next_biz_date		= " + next_biz_date);
		PrintLog("one_m_back		= " + one_m_back);
		PrintLog("three_m_back		= " + three_m_back);
		PrintLog("six_m_back		= " + six_m_back);
		PrintLog("year_back			= " + year_back);
		PrintLog("posi_pay_purge	= " + posi_pay_purge);
		PrintLog("chex_log_purge	= " + chex_log_purge);
		PrintLog("acct_log_purge	= " + acct_log_purge);
		PrintLog("posi_log_purge	= " + posi_log_purge);
		PrintLog("stop_log_purge	= " + stop_log_purge);
		PrintLog("holidays_purge	= " + holidays_purge);
		PrintLog("eod_oper			= " + eod_oper);
	}
