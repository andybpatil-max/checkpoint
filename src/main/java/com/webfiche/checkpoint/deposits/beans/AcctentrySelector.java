...
public final class AcctentrySelector implements Serializable {
	/**
	 * 
	 */
	private static final long	  serialVersionUID  = 1L;
	// -- Instance Variables
	private String	className		= "> AcctentrySelector.";
	...
	// Key PayerAba value RDC, LOCKBOX, NEXTDAY
	...
	public void setAcctList(TreeMap<String, AccountDetail> acctList) {
		...
		*/
		...
		PrintLog("Size of acctlist: "+acctList.size());
	}
	public TreeMap<String, AccountDetail> getAcctList() {
		return (this.acctList);
	}
	public AccountDetail getAcctRow (String acctKey) {
		//PrintLog("AcctKey: "+acctKey);
		//if (acctList.containsKey(acctKey)) {
		//	PrintLog("Value of "+acctKey+" is: "+acctList.get(acctKey).getAccount_availday3());
		//}
		return (this.acctList.get(acctKey));
	}
	public void ListAccounts () {
		Set<String> keys1 = acctList.keySet();
		for(String key: keys1){
			PrintLog("Value of "+key+" is: "+acctList.get(key).getAccount_availday3());
		}
	}
	//
	...
	*/
	//
	public AcctentryDetail[] getPostingRows() {
		//moduleName				= "getPostingRows: ";
		AcctentryDetail results[]	= new AcctentryDetail[postingRows.size()];
		Iterator<AcctentryDetail> posting	= postingRows.iterator();
		int n = 0;
		while (posting.hasNext()) {
			results[n++] = (AcctentryDetail) posting.next();
		}
		// PrintLog("Detail count "+n);
		return (results);
	}
	//
	public ArrayList<AcctentryDetail> getPostingRowsArray() {
		return (this.postingRows);
	}
	//
	public AcctentryDetail getArow() {
		AcctentryDetail results = new AcctentryDetail();
		Iterator<AcctentryDetail> chex = postingRows.iterator();
		results = (AcctentryDetail) chex.next();
		return (results);
	}
	//
	public AcctentryDetail getArow(int recIndex) {
		AcctentryDetail aRow	= new AcctentryDetail();
		// Iterator chex = postingRows.iterator();
		this.recIndex = recIndex;
		aRow	= (AcctentryDetail) postingRows.get(recIndex);
		return (aRow);
	}
	//
	public void setPostingRows(ArrayList<AcctentryDetail> postingRow) {
		postingRows.clear();
		Iterator<AcctentryDetail> rows = postingRow.iterator();
		int n = 0;
		while (rows.hasNext()) {
			this.postingRows.add(n, (AcctentryDetail) rows.next());
			n++;
		}
		//PrintLog("Checks Count: "+n);
		detail_count = n;
